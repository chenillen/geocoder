...
Install either as a plugin:
  ...
  script/plugin install git://github.com/chenillen/geocoder.git

=== b. Rails 3

Install either as a plugin:

  rails plugin install git://github.com/chenillen//geocoder.git
...
E) Optionally, auto-fetch coordinates, formatted_address, geometry and location_type every time your model is saved:

	after_validation :fetch_all

<i>Note that you are not stuck with the +latitude+, +longitude+, +formatted_address+, +geometry+ and +location_type+ column names, or the +address+ method. See "More On Configuration" below for details.</i>
...
If you have other columns configured, you can use fetch_all and fetch_all! methods
	obj.fetch_all              			   # fetches and assigns all information get from Google Map
  obj.fetch_all!            				 # also saves lat, lon, formatted_address, geometry, location_type attributes
...
You are not stuck with using the +latitude+ and +longitude+ database column names for storing coordinates. For example, to use +lat+ and +lon+:
  ...
  geocoded_by :address, :latitude  => :lat, :longitude => :lon, :formatted_address => :addr,
        			:geometry => :geometry, :location_type => :location_type
