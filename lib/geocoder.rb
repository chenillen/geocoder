...
module Geocoder
  ...
  def self.lat_lng_reg_match(latlng)
    ...
    # /^-?([1-8]?[1-9]|[1-9]0)\.{1}\d{1,18},-?([1]?[1-7][1-9]|[1]?[1-8][0]|[1-9]?[0-9])\.{1}\d{1,18}$/
    latlng_reg = /^-?([1-8]?[1-9]|[1-9]0)\.{1}\d{1,18},-?([1]?[1-7][1-9]|[1]?[1-8][0]|[1-9]?[0-9])\.{1}\d{1,18}$/
    latlng = latlng.split(',').map{|l| l.strip}.join(',')
    latlng =~ latlng_reg ? true : false
  end
end
